...
public:
    ...
    void message_id_change_callback()
    {
        if (get_server_state() == Idle && get_client_state() == Transmitting)
        {
            ...
            server_current_endpoint_char->setValue(endpoint);
            if (text_to_text_callbacks.count(endpoint) > 0)
            {
                const std::string response = text_to_text_callbacks[endpoint]->callback(rx_char->getValue());
                tx_char->setValue(response);
                server_state_char->setValue(Transmitting);
            }
        }
    }
    ...
    void run()
    {
        ...
        if (uploadInProgress)
        {
            ...
            {
                int32_t myArray[50];
                for (int i = fileUploadIndex; i < (fileUploadIndex + 200) && i < (fileUploadIndex + fileLength); i++) {
                    myArray[i] = EEPROM.read(i);
                }
                // Read the next packet
                // Send the packet to the frontend
                uint8_t *data = reinterpret_cast<uint8_t *>(myArray);
