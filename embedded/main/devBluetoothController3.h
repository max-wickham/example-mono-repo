...
public:
    void setup()
    {
        ...
        configure_characteristic(client_service, client_rx_char, client_props, CLIENT_RX);
    ...
    void run()
    {
        if (upload_state.uploading)
        {
            ...
            else
            {
                ...
                int i = index;
                const int packetLength = 200 > upload_state.file_length - index ? upload_state.file_length - index : 200;
                ...
                uint8_t *data = reinterpret_cast<uint8_t *>(&packet[0]);
